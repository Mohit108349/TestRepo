...
import com.familypedia.databinding.FragmentPostRequestBinding
...
class PostRequestFragment : Fragment(), PermissionsListener, FamilyPediaClickListener,
    SuccessFulListener {
    ...
    private fun setAdapter() {
        postRequestAdapter =
            PostRequestParentAdapter(requireContext(), permissionList, FROM_POST_REQUEST, this)
        binding.rvPostRequest.adapter = postRequestAdapter
    }

    override fun onRequestAccept(data: AskCharacterPermissionRequest) {
        acceptDeclineRequest(true, data)
    }

    override fun onRequestReject(data: AskCharacterPermissionRequest) {
        acceptDeclineRequest(false, data)
    }
    ...
    private fun acceptDeclineRequest(
        ...
        permissionsViewModel?.acceptDeclinePermission(requireContext(), acceptRequest, request)
    }
    ...
    private fun responseData() {
        permissionsViewModel?.permisssionResponseResult?.observe(requireActivity(), { response ->
            if (response.data?.data != null && response.data.data?.isNotEmpty() == true) {
                ...
                response.data.data.forEach { permissionData ->
                    if (permissionData.characterEditDeleterequestUser != null && permissionData.characterEditDeleterequestUser?.isNotEmpty() == true) {
        ...
        permissionsViewModel?.simpleResponseResult?.observe(requireActivity(), { response ->
            if (response != null) {
                if (response.apiName == ACCEPT_CHARACTER_PERMISSION || response.apiName == DECLINE_CHARACTER_PERMISSION) {
    ...
    private val TAG = "PostRequestFragment"
    private fun manageDataLoadingStatus() {
        permissionsViewModel?.updateLoaderStatus?.observe(viewLifecycleOwner, { loadingData ->
            if (loadingData.apiName == CHARACTER_PERMISSION_LIST) {
                ...
                Log.d(TAG, "manageDataLoadingStatus: ${loadingData.apiName} , ${loadingData.status}")
                if (loadingData.status) {
    ...
    private fun manageSomethingWentWrong(errorType: ErrorType) {
        ...
        if (errorType == ErrorType.NO_DATA_AVAILABLE) {
            ...
            binding.somethingWrong.tvSomethingWentWrong.setTextOnTextView(getString(R.string.no_post_request_available) , "")
